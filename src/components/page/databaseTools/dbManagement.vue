...
<template>
    <div :style="{height:(screenSize.height)+'px'}">
        <dv-loading v-if="fullscreenLoading" style="color: white;">Loading...</dv-loading>
        <el-row :gutter="10" style="height:100%;" v-if="!fullscreenLoading">
                ...
                <dv-border-box-13 class="dvBox8">
                    <div class="leftBox">
                        <el-tree :data="listData.content" node-key="id" :expand-on-click-node="false" ref="tree"
                            ...
                            <div class="custom-tree-node" slot-scope="{ node, data }" @click="goEdit(data)">
            ...
            <el-col :span="18" style="height:100%">
                <dv-loading v-if="viewDialog.isLoading" style="color: white;">Loading...</dv-loading>
...
<!--                                <div
                        ...
                        <div v-if="data.level != 2 && !viewDialog.isTools">
                            ...
                            >
                                ...
                                <el-table-column label="原字段名" width="200">
                                    <template scope="scope">
                                        <el-input
                            ...
                            <el-form :model="data" v-loading="viewDialog.butIsLoading" :rules="rules" ref="ruleForm">
                                <div>
                                    ...
                                    <el-row :gutter="20">
                                        ...
                                        <el-col :span="8" class="frame-col">
                                            <el-button class="frame-but" type="primary" plain @click="copyEntity">实体
                                            </el-button>
                                        </el-col>
                                        <el-col :span="8" class="frame-col">
...
export default {
    ...
    methods: {
        ...
        goEdit(row) {
            ...
            this.viewDialog.isTools = false;
            this.viewDialog.isLoading = true;
            this.getHttp("/api/databaseTools/dataSource/editView?dataSourceId=" + row.id + "&tableName=" + row.label).then((result) => {
                this.viewDialog.isShow = true;
                this.viewDialog.isLoading = false;
                this.data = result;
        ...
        sysColumn() {
            var row = this.data.detailEntitys[this.currentIndex];
            this.postHttp("/api/databaseTools/dataSource/sysColumn", row).then((result) => {
                this.$message({message: '同步成功', type: 'success'});
                this.goEdit({'id': this.data.dataSourceId, 'label': this.data.number, 'name': this.data.name});
            });
        },
        //复制实体
        copyEntity() {
