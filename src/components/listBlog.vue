...
export default {
  ...
  methods: {
    listAllBlog() {
      this.$http.get("/api/student/findAllPageByParams?page=1&size=5&sort=id,ASC").then(result => {
    ...
    search() {
      this.$http.get("/findAllPageByParams?page=1&size=5&sort=id,ASC").then(result => {
        var result = result.body;
        if (result.code === 200) {
          this.list = [];
